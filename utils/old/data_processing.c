...
int main(int argc, char **argv)
{
    ...
    // The steps-dependent
    double *MSD = (double *) calloc(arguments.N_maxsteps - arguments.skip, sizeof(double));
    if (MSD == NULL)
    {
        perror("Allocating the MSD");
        goto RDF;
    }

    double *RMSD = (double *) malloc((arguments.N_maxsteps - arguments.skip) * sizeof(double));
    if (RMSD == NULL)
    {
        perror("Allocating the RMSD");
        goto MSD;
    }
    ...
    if (initial_upositions == NULL)
    {
        perror("Allocating the initial unwrapped positions");
        goto RMSD;
    }

    double **current_upositions = (double **) malloc(N_atoms * sizeof(double *));
    if (current_upositions == NULL)
    {
        perror("Allocating the current upositions");
        goto IUPOSITIONS;
    }
    ...
    for (int a = 0 ; a < N_atoms ; a++)
    {
        ...
        if (initial_upositions[a] == NULL)
        {
            perror("Slot of initial unwrapped position");
            goto CPOSITIONS;
        }

        current_upositions[a] = (double *) malloc(3 * sizeof(double));
        if (current_upositions[a] == NULL)
        {
            perror("Slot of current unwrapped position");
            goto CPOSITIONS;
        }
    ...
    for (int a = 0 ; a < N_atoms ; a++)
    {
        ...
        atom_index--;
        if (fscanf(conf_file_ptr, "%*f %*f %*f %lf %lf %lf", &(initial_upositions[atom_index][0]),
                                                             &(initial_upositions[atom_index][1]),
                                                             &(initial_upositions[atom_index][2])) != 3)
        {
            errno = EIO;
            perror("Reading the initial unwrapped positions");
            goto CONF_FILE;
        }
        ...
        {
            errno = EIO;
            perror("Dumping the rest of the lines");
            goto CONF_FILE;
        }
    ...
    for (int step = 0 ; step < arguments.N_maxsteps ; step++)
    {
        ...
        for (int a = 0 ; a < N_atoms ; a++)
        {
            ...
            atom_index--;
            if (fscanf(conf_file_ptr, "%lf %lf %lf %lf %lf %lf", &(current_positions[atom_index][0]),
                                                                 ...
                                                                 &(current_upositions[atom_index][2])) != 6)
            {
                err = EIO;
            ...
            if (step >= arguments.skip)
            {
                ...
                for (int d = 0 ; d < 3 ; d++)
                {
                    double diff = current_upositions[a][d] - initial_upositions[a][d];
                    MSD[step - arguments.skip] += diff * diff;
                }
            }
        }
        ...
        if (step >= arguments.skip)
        {
            MSD[step - arguments.skip] /= (double) N_atoms;
    ...
    // Computing the RMSD for each configuration and averaging the MSDs
    for (int step = 0 ; step < arguments.N_maxsteps - arguments.skip ; step++)
    {
        RMSD[step] = (double) sqrt(MSD[step]) / (arguments.N_maxsteps - arguments.skip);
        MSD[step] /= (double) arguments.N_maxsteps - arguments.skip;
    }
    ...
    CUPOSITIONS: free(current_upositions);
    IUPOSITIONS: free(initial_upositions);
    RMSD: free(RMSD);
    MSD: free(MSD);
    RDF: free(RDFs);
